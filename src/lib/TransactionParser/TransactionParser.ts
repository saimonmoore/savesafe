...
class TransactionParser {
  ...
  public async handleWorkerMessage(
    event: MessageEvent<UploadEventPayload>
  ): Promise<void> {
  ...
  private getTranslatedHeaders(headers: string[]): Record<string, string> {
    return headers.reduce(
      (translatedHeaders: Record<string, string>, header: string) => {
        const standardHeader = this.standardizeHeader(header);
        const translatedHeader =
          TransactionColumnNamesMap[
            standardHeader as keyof typeof TransactionColumnNamesMap
          ];
  ...
  private getMappingFromTranslatedHeaders(
    translatedHeaders: Record<string, string>,
    delimiter: string
  ): CSVMapping {
    const requiredHeaders = [
      TransactionColumnNames.AMOUNT,
      TransactionColumnNames.DESCRIPTION,
    ];
    ...
    if (
      !translatedHeaders[TransactionColumnNames.TRANSACTION_DATE] &&
      !translatedHeaders[TransactionColumnNames.EFFECTIVE_DATE]
    ) {
  ...
  private getMapping(headers: string[], delimiter: string): CSVMapping {
    ...
    const mapping = this.getMappingFromTranslatedHeaders(
      translatedHeaders,
      delimiter
    );
      ...
      for (let i = headerLineIndex + 1; i < lines.length; i++) {
        ...
        try {
          ...
          const transactionDate =
            fields[headerIndexMap[mapping.transactionDate]];
